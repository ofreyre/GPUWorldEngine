...
float4x3 sum4x3(float4x3 m, float3 q)
{
    return float4x3(
        m._11_12_13 + q,
        m._21_22_23 + q,
...
float4x3 sub4x3(float4x3 m, float3 q)
{
    return float4x3(
        m._11_12_13 - q,
        m._21_22_23 - q,
...
float4x3 easeInOut4x3(float4x3 interpolator)
{
...
{
    float4x3 cell = sum4x3(cp, direction);
    ...
    for (int z = 0; z <= 1; z++)
    {
        ...
        {   
            ...
            for (int x = 0; x <= 1; x++)
            {
                ...
                //Perlin
                float4x3 cdP = rand(modulo(sum4x3(floor(vP), direction), peP)) * 2.0 - 1;
                float4x3 dP = sub4x3(fP, direction);
                nX[x] = dot4x3(cdP, dP);
...
float2x4 CelularPerlin3D(float3 p)
{
    ...
    float4 frequencyC = 1;
    float2x4 factor = 1;
    uint layers = min(16, max4(_LayersP, _LayersC));
    float4x3 pP = float4x3(p, p, p, p);
    float4x3 pC = pP;
    float2x4 roughness = float2x4(_RoughnessP, _RoughnessC);
    ...
    for (uint i = 0; i < layers; i++)
    {
        noise += mul2x4(
            PerlinCellular3Tiled(
                mul4x3_4(pP, frequencyP), _CellLengthP, step(i, _LayersP),
                mul4x3_4(pC, frequencyC), _CellLengthC, step(i, _LayersC),
                _Period),
            factor);
        frequencyP *= _PersistanceP;
        frequencyC *= _PersistanceC;
...
float4 Nose(float3 p)
{
    uint max = uint(_Period);
    float2x4 noise = CelularPerlin3D(p);
    float4 channelFilter = step(0, _Weight);
    return (noise._11_12_13_14 * _Weight + (1 - noise._21_22_23_24) * (1 - _Weight)) * channelFilter;
}
